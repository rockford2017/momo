...
async def safe_type(chat_id, seconds=2):
    """شبیه‌سازی تایپینگ انسان جهت جلوگیری از ریپورت"""
    try:
...
async def loop_sender_task():
    ...
    global IS_LOOP_ON
    while True:
        if IS_LOOP_ON and TARGET_CHAT_LOOP and TEXT_LOOP:
            try:
                await safe_type(TARGET_CHAT_LOOP, 3)
                sent_msg = await app.send_message(TARGET_CHAT_LOOP, TEXT_LOOP)
                
                # پاک‌سازی خودکار ردپا پس از ۲ دقیقه
                asyncio.create_task(delete_trace(sent_msg, 120))
            except Exception as e:
                print(f"Error in Loop: {e}")

            delay = INTERVAL_LOOP + random.randint(5, 30)
...
async def show_help(client, message):
    ...
    help_text = (
        f"📋 **داشبورد و راهنمای سلف‌بات مشتی**\n"
        f"💎 ───────────────────────── 💎\n\n"
        f"⚙️ **وضعیت فعلی سیستم:**\n"
        f"├ 🔄 **حلقه ارسال:** {st_loop}\n"
...
async def toggle_pmbot(client, message):
    global IS_PMBOT_ON
    cmd = message.text.split()
    IS_PMBOT_ON = True if (len(cmd) > 1 and cmd[1] == "on") else False
...
async def toggle_antidel(client, message):
    ...
    cmd = message.text.split()
    IS_ANTIDEL_ON = True if (len(cmd) > 1 and cmd[1] == "on") else False
...
async def pv_handlers(client, message):
    if IS_AFK_ON:
        ...
        await message.reply_text("⚠️ **پیام شما دریافت شد.** لطفاً شکیبا باشید.")

    # ذخیره‌ساز هوشمند رسانه‌های یک‌بارمصرف (TTL)
